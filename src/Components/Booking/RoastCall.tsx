...
interface RoastCallBookingProps {
  prefillData?: {
    name?: string;
    email?: string;
    phone?: string;
...
export default function RoastCallBooking({
  prefillData,
}: RoastCallBookingProps) {
  const router = useRouter();

  useEffect(() => {
    (async function () {
      const cal = await getCalApi({ namespace: "roast" });
  ...
  return (
    <Cal
      namespace="roast"
      calLink="jerometana/roast"
      style={{ width: "100%", height: "100%", overflow: "scroll" }}
      config={{
        name: prefillData?.name || "",
        email: prefillData?.email || "",
        phone: prefillData?.phone || "",
        currentWebsiteUrl: prefillData?.currentWebsiteUrl || "",
        layout: "month_view",
        theme: "light",
      }}
    />
